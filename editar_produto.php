...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            $stmt = $conn->prepare("UPDATE produtos SET nome = ?, descricao = ?, preco = ?, quantidade_estoque = ?, imagem_url = ?, disponivel = ? WHERE id = ?");
            $stmt->execute([$nome, $descricao, $preco, $quantidade, $imagem, $disponivel, $id]);
            
            header("Location: telaadministrativa.php?success=2");
            exit();
        }
    }
            ...
            <div class="col-md-8">
                ...
                <form method="POST">
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="disponivel" name="disponivel" <?php echo ($produto['disponivel'] ? 'checked' : ''); ?>>
